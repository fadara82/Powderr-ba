(function ($) {
  $.spapp = function (options) {
    var config,
      routes = {},
      currentView = null; // prati aktivni view da ne poziva duplo

    config = $.extend(
      {
        defaultView: $("main#spapp > section:last-child").attr("id"),
        templateDir: "./tpl/",
        pageNotFound: false,
      },
      options
    );

    $("main#spapp > section").each(function (k, e) {
      var elm = $(this);
      routes[elm.attr("id")] = {
        view: elm.attr("id"),
        load: elm.data("load"),
        onCreate: function () {},
        onReady: function () {},
      };
    });

    this.route = function (options) {
      $.extend(routes[options.view], options);
    };

    var routeChange = function () {
      var id = location.hash.slice(1);

      if (id === currentView) {
        // Ako je ista ruta kao prije, ne radimo ništa
        return;
      }
      currentView = id;

      var route = routes[id];
      var elm = $("#" + id);

      if (!elm.length || !route) {
        if (config.pageNotFound) {
          window.location.hash = config.pageNotFound;
          return;
        }
        console.log(id + " not defined");
        return;
      }

      if (elm.hasClass("spapp-created")) {
        route.onReady();
      } else {
        elm.addClass("spapp-created");
        if (!route.load) {
          route.onCreate();
          route.onReady();
        } else {
          elm.load(config.templateDir + route.load, function () {
            route.onCreate();
            route.onReady();
          });
        }
      }
    };

    this.run = function () {
      if (!window._spappHashListenerAdded) {
        window.addEventListener("hashchange", function () {
          routeChange();
        });
        window._spappHashListenerAdded = true;
      }
      if (!window.location.hash) {
        window.location.hash = config.defaultView;
      } else {
        routeChange();
      }
    };

    return this;
  };
})(jQuery);
